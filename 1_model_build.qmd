...
This reached the best possible R^2 score of 1.0 (!). However, the testing dataset might not be the best since I splitted training and testing data based on max phase... rather than the usual train_test_split(), so the testing data might not be the best candidates to test the model. Ideally, I should look for a completely different set of compounds that were outside of ChEMBL database. Anyway, for demonstration purpose, I'd stick to what I've for now.

---
title: "Random forest"
subtitle: "Series 2.2 - building model, feature importances & model evaluations"
...
This step was actually recommended to happen prior to checking out feature importances, as suggested by *scikit-learn* in the [Permutation feature importance section](https://scikit-learn.org/stable/modules/permutation_importance.html#permutation-feature-importance).
...
* Prior to using feature importances, need to check accuracy of trained model via the score() method.

```{python}
rfreg.score(X_mp_test, y_mp_test)
```

This reached the best possible R^2 score of 1.0 (!). The likely reason could be that I had a really small training dataset (imagining only training on 10 compounds...). The main reason to have such a small training dataset was that that were basically the only 10 compounds I've found with an assigned max phase of 4, which were equivalent to approved prescription drugs with acetylcholinesterase as the main protein drug target.

***May need to add a mean squared error for y_mp4 & y_mp_test (= rfreg.predict(X_mp_test))***

<br>

##### **Feature importances**

If going for only of what were available from *scikit-learn*, there were 2 types of feature importances available, which I've described below.
...
- ?number of samples (max_samples) (probably won't do this as the sample size was already small to start with!)

...
```{python}
#data["max_phase"].replace({"null": 0}, inplace=True)
```
...
# data_ro = iblr.ro(data = data, y = "pKi", samp_method="extreme")
# print(data_ro.shape)

# Using random under-sampling
# data_ru = iblr.random_under(data = data, y = "pKi")
# print(data_ru.shape)

#Gaussian noise
data_ro = iblr.gn(data = data, y = "pKi", pert = 1)
print(data_ro.shape)
```
...
```{python}
# under-sampling - nil max phase 4 compounds!
# data_ru_mp4 = data_ru[data_ru["max_phase"] == 4]
# data_ru_mp4
# print(data_ru_mp4.shape)
```
...
```{python}
# under-sampling
# data_ru_mp_null = data_ru[data_ru["max_phase"] == "null"]
# data_ru_mp_null
# print(data_ru_mp_null.shape)
```
...
sns.kdeplot(data_ro["pKi"], label = "Modified")
plt.legend(labels = ["Original", "Modified"])
```
...
rfreg trained model used max phase splits only.

...
Here, I'd like to introducing another package, scikit_mol, which actually originated from 2022 RDKit UGM hackathon. This [blog post](https://www.cheminformania.com/scikit-mol-easy-embedding-of-rdkit-into-scikit-learn/) added more explanations and code about it. However, for this post I've only used it for the purpose of checking for missing SMILES or errors in SMILES. It has other functions e.g. integrating with scikit-learn's pipeline method for multiple estimators. Its GitHub Repo: https://github.com/EBjerrum/scikit-mol
...
It appeared to have no unparsable (or missing) molecules.
...
```{python}
# Dropping duplicated compound via chembl IDs
...
Eventually df_full contained 10 active compounds and 5256 inactive compounds.
...
# max_features="sqrt" by default, but in case I did anything weird, I decided to show it explicitly in the code to make sure the right one was used
...
5. two approaches from GHOST paper (main idea was optimising and shifting decision threshold):
    ...
    - approach 1 (out-of-bag approach) based on RDKit blog post (ref. 41) - http://rdkit.blogspot.com/2018/11/working-with-unbalanced-data-part-i.html

    - approach 2 led to Generalised threshold shifting (GHOST) procedure - ghostml package
...
6. shift decision threshold using ghostml, a postprocessing way (note: last post used re-sampling method in a preprocessing way)

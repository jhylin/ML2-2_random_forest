...
# Using df_merge_new
# df_merge_new = df_merge_new.assign(max_phase_lb = df_merge_new["max_phase"] / 4)
...
# first_col = df_merge_new.pop("max_phase_lb")
# df_merge_new.insert(0, "max_phase_lb", first_col)
...
# for df merging after

```

```{python}
# Combine df_mp4_lb & df_null

# df_full = pd.merge(
#     df_null[["max_phase_lb"]],
#     df_mp4_lb,
#     left_index=True,
#     right_index=True
# )
...
**TODO**:
**Combine df_null & df_mp4_lb (= df_full) and use train_test_split() at test_size = 0.2**

```{python}
# Do train_test_split() on df_merge_new/df_full (mixture of max_phase 0 and 1)
...
```{python}
# To avoid re-inventing the wheel, the following function code for calculating the metrics of the RF model was adapted from Landrum et al., from this notebook: https://github.com/rinikerlab/GHOST/blob/main/notebooks/example_GHOST.ipynb
...
```{python}
# Problem with y_test - only one label in it (1 only), likely needs binary labels (0 & 1)
# needed for roc_auc_score

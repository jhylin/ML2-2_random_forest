...
##### **ROC curves before and after shifting threshold**
...
Alternative ROC curve plotting method:

* Curve looks weird at the moment - work-in-progress
...
```{python}
# Set up true +ve rate (tpr), false +ve rate (fpr) & threshold
fpr, tpr, thresh = metrics.roc_curve(y_train, train_probs, pos_label=1.0)
```
...
```{python}
pt = roc_df.plot(x = "specificity", y = "recall", figsize=(3, 3), legend=False)

# random guessing (threshold = 0.5)
pt.plot([0, 1], [0, 1], color = "b", lw = 2, linestyle="--")

pt.set_ylim([0.0, 1.05])
pt.set_xlim([0.0, 1.0])

pt.set_xlabel("specificity")
pt.set_ylabel("recall")
```
...
# # random guessing (threshold = 0.5)
# plt.plot([0, 1], [0, 1], color = "", lw = 2, linestyle="--")

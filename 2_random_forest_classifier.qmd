...
*Introducing new packages - chembl_downloader, random_froest.py & GHOST paper*
...
- aim to model and classify the max phase of ChEMBL small molecules (max phase 4 or not)
    - *target*: max_phase
    - *features*: various RDKit 2D descriptors

- (optional) re-label max phases as binary labels (e.g. max phase null as 0, max phase 4 as 1) 
    - may not need to do this, as can be more broadly about finding important features targeting towards max_phase in general (ranges from unassigned, 0, 1, 2, 3, 4)

- train/test splits using ghostml (imbalanced dataset of actives and inactives)
...
- 2 approaches from GHOST paper (main idea was on shifting decision threshold)
    ...
    *possible to use only approach 2 ghostml - as both approaches were shown to be performing similarly in the paper*

    - include data splits/threshold shifting method

- Using same set of training data to train RandomForestClassifier() in scikit-learn 

* Visualisations - ?feature importances or others